...
For projects that use Nix, consume the rules as a versioned flake input and pick rules by name. The upstream exposes a `rules` catalog (name → file), a `lib.materializeRules` helper, and a `lib.lintRules` validator; commit the generated `.clinerules/*.generated.md` so a plain `git clone` works without Nix.
